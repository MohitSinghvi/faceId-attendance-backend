...
app.post('/addStudent', (req, res) => {
  ...
  con.query('insert into student values (?,?,?,?)',[rollNo,name,course,batch] ,function (err, result) {
...
app.post('/mark-attendance', (req, res) => {
  ...
  rekognition.searchFacesByImage(params, (err, data) => {
    if (err) {
      res.status(500).json({ error: 'Error searching for faces in collection' });
    } else {
      if(data?.FaceMatches?.length>0){
        con.query('insert into attendance values (?,?,?)',[id, sessionId, data?.FaceMatches[0]?.Face?.ExternalImageId] ,function (err, result) {
          if (err) res.send(JSON.stringify({
              statusCode: 500,
              body: JSON.stringify(err),
          }));
          res.status(200).json({ message: 'Attendance Marked', rollNo: data?.FaceMatches[0]?.Face?.ExternalImageId });
        });
      } else {
